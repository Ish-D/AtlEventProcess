...
int main(int argc, char* argv[])
{
    ...
    if (argc < 3) {
        std::cerr << "usage: " << argv[0]
                  << " [-d --deflate] -e [--event] <eventNumbers> [-r, --run <runnumber>] [-l, --listevents] [-t --checkevents] -o, --out outputfile "
                     "inputfiles...."
                  << std::endl;
    ...
    for (const std::string& fName : fileNames) {
        ...
        eventLoop(std::move(pDR), std::move(pDW), nFound, searchEvents, searchRun, searchRunSet, listEvents, checkEvents);
